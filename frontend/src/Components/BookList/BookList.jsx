...
const toggleFavorite = (id) => {
  ...
  return (
    <div className="list-container">
      {/* for Seatrch */}
      <form 
      className="d-flex  col-10 p-4 " role="search">
        <input
      ...
      <table id="table" className="table table-striped">
        <thead className="table-dark">
          <tr> 
            <th  onClick={() => sortBookData('title')} scope="col"className="search" >            
              Title</th>
            <th onClick={() => sortBookData('authors')} scope="col"className="search">
            <img style={{backgroundColor:'white'}} src="./sort-alpha-up.svg" alt=""/>
              Author</th>
            <th  onClick={() => sortBookData('publisher')} scope="col" className="search">&nbsp;
            <img style={{backgroundColor:'white'}} src="./sort-alpha-up.svg" alt=""/>
              Publisher</th>
          ...
          .filter((books) => {
            ...
            .map((books) => (
              <tr key={books._id}>               
                ...
                <td>
           <GrFavorite checked = {books.favorite}
            style={{ backgroundColor: favorite ? "green" : null }}
           ...
           />
                ...
                <td>
                  <Link 
                    onClick={() => setShowForm(true)}
                    {...(showForm && <AddBook />)}
                    className="btn btn-success" id="btnList"
                    to={`/edit/${books._id}`}
                  >
                    <FiEdit />
                  </Link>
                  ...
                  className="btn btn-danger">&nbsp;<FaBoxArchive/> </button>
                </td> 
